...
class prediction{
	...
	public function sppm_regression()
	{
		...
		/*echo Accuracy::score(, $mypredval);
		echo "</br>";
		echo Accuracy::score(, $mypredval, false);*/
	}
		...
		try
		{
			if($std_id != '')
			{
				...
				if($std != '')
				{
					...
					//$actualLabels = $std;
					$predictedLabels = $myroundval;

					//echo round(Accuracy::score($std, $mydata),2);
					// return 0.75
					//echo "</br>";
					//echo round( Accuracy::score($std, $mydata, true),2);
					// return 3
					...
					if($myroundval == 1)
					{
						echo "</br> A and Accuracy of grade is ".round(Accuracy::score($std, $mydata),2)*100;
					}
					else if($myroundval == 2)
					{
						echo "</br> B and Accuracy of grade is ".round(Accuracy::score($std, $mydata),2)*100;
					}
					else if($myroundval == 3)
					{
						echo "</br> C+ and Accuracy of grade is ".round(Accuracy::score($std, $mydata),2)*100;
					}
					...
					{
						echo "</br> F and Accuracy of grade is ".round(Accuracy::score($std, $mydata),2)*100;
					}
