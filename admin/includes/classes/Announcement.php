...
class announcement{
	...
	///edit announcements
	public function edit_announcement($announce,$inst_name)
	{
		$id=$announce["id"];
		$ann_title = $announce["ann_title"];
		$cur_date=date("Y-m-d h:i:sa");
		...
		$query="UPDATE `announcement` SET `ann_title`='$ann_title',`ann_date`='$ann_date',`dep_title`='$dep_title',`course_id`='$course_id',`ann_desc`='$ann_desc',`inst_name`='$inst_name' WHERE `ann_id`='$id' ";
		if($this->con->query($query))
		{
